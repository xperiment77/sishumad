...
class LayananController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("layanan.id", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("layanan.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Layanan";
		}
		else{
			if($db->getLastError()){
				$this->set_page_error();
		...
		return $this->render_view("layanan/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		...
		$page_title = $this->view->page_title = "Add New Layanan";
	...
	function delete($rec_id = null){
		...
		$db->where("layanan.id", $arr_rec_id, "in");
